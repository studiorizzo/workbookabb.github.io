...
function renderConfigurazione(content) {
    ...
    const inizioCorrente = metadata.inizio_corrente || (config[8]?.[6] || '2024-01-01');
    const fineCorrente = metadata.fine_corrente || (config[8]?.[5] || '2024-12-31');
    const annoCorrente = metadata.anno_esercizio || (config[8]?.[7] || new Date().getFullYear());
    
    // Date esercizio precedente (da config[9])
    const inizioPrecedente = metadata.inizio_precedente || (config[9]?.[6] || '2023-01-01');
    const finePrecedente = metadata.fine_precedente || (config[9]?.[5] || '2023-12-31');
